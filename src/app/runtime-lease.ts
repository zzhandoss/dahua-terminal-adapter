...
import type { AdapterLease } from "../contracts/device-service.js";
import type { DeviceServiceClient } from "../infra/ds/device-service-client.js";
import {
  DEVICE_SETTINGS_SCHEMA_VERSION,
  deviceSettingsSchema
} from "../domain/devices/device-settings-schema.js";
  ...
  while (input.isRunning()) {
    try {
      ...
      const delayMs = Math.min(
        REGISTER_RETRY_BASE_MS * 2 ** Math.min(attempts - 1, 8),
...
export function shouldReRegister(error: unknown): boolean {
