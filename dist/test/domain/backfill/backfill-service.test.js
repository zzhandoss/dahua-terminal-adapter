...
describe("BackfillService", () => {
    it("returns local events when local already satisfies limit", async () => {
        ...
        const service = new BackfillService(makeStore(local), { get: () => null }, makeClientFactory([]), pino({ level: "silent" }));
    ...
    it("merges access-control records and uses stable hash event ids", async () => {
        ...
        const service = new BackfillService(makeStore(local), { get: () => makeAssignment() }, makeClientFactory([
    ...
    it("returns deterministic hash ids for remote records", async () => {
        const service = new BackfillService(makeStore([]), { get: () => makeAssignment() }, makeClientFactory([
...
function makeStore(events) {
    return {
        getBackfill(deviceId, _sinceEventId, limit) {
            if (deviceId !== "dev-1") {
                return [];
            }
...
function makeClientFactory(records) {
    return {
        create() {
            return {
                client: {
