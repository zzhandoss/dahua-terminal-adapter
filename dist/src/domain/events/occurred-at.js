...
export function resolveOccurredAt(event, now = Date.now()) {
    return resolveOccurredAtWithPolicy(event, { mode: "boundedDevice", maxDriftMs: 60000 }, now).occurredAt;
}
export function resolveOccurredAtWithPolicy(event, policy, now = Date.now()) {
...
export function resolveOccurredAtFromAccessEvent(event, policy, now = Date.now()) {
    if (policy.mode === "adapter") {
        return now;
    }
...
function readDataField(event, key) {
