...
const allowedTemplateKeys = new Set(DAHUA_FIND_PERSON_TEMPLATE_KEYS);
const IDENTITY_SOURCE_VALUES = ["accessUser", "accessCard"];
...
export function parseDahuaSettings(settingsJson) {
    if (!settingsJson) {
    ...
    try {
        ...
        throw new Error(`invalid settingsJson: ${error.message}`);
    }
    return DeviceSettingsSchema.parse(raw);
}
export function toDahuaConnectionConfig(settings, defaults) {
    return {
