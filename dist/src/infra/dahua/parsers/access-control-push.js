import { toIntOrNull, toObjectOrNull, toStringOrNull } from "../../../shared/parse.js";
export function parseEventUploadBody(input) {
...
export function parseImageEventUploadBody(input) {
    if (!input || typeof input !== "object") {
        return null;
    }
    const body = input;
...
export function parseTextPlainUploadEvent(input) {
...
export function parseTextPlainUploadToJson(input) {
    const raw = input.trim();
    if (!raw) {
...
export function toRawDahuaEvent(event) {
    return {
        code: event.Code,
        action: event.Action,
        index: event.Index,
        ...
        if (Array.isArray(nested)) {
            nested.forEach((item, index) => {
                if (item && typeof item === "object" && !Array.isArray(item)) {
                    flattenObject(item, `${next}[${index}]`, out);
                }
                else {
...
function readFieldFromTail(input, key) {
...
function parseMaybeJson(valueRaw) {
