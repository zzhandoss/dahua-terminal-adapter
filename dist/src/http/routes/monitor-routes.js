...
export function registerMonitorRoutes(app, runtime) {
    app.get("/health", async (_request, reply) => reply.send(ok({ status: "ok" })));
    app.get("/:uploadType/keepalive", async (request, reply) => {
        const uploadType = String(request.params.uploadType ?? "");
