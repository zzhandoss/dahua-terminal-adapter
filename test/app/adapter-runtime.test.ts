...
import type { AdapterLease } from "../../src/contracts/device-service.js";
...
import { PermanentDeviceServiceError, type DeviceServiceClient } from "../../src/infra/ds/device-service-client.js";
import type { EventNormalizationPipeline } from "../../src/domain/events/event-normalization-pipeline.js";
...
type RegisterFn = (input: unknown) => Promise<AdapterLease>;

describe("AdapterRuntime start", () => {
  it("sends deviceSettingsSchema in register payload", async () => {
    const register = vi.fn<RegisterFn>().mockResolvedValue(makeLease());
    const dsClient = makeDsClient(register);
    const runtime = new AdapterRuntime(
      makeEnv(),
      dsClient as unknown as DeviceServiceClient,
      makeStoreStub() as unknown as SqliteStore,
      {} as EventNormalizationPipeline,
      makeLoggerStub()
    );
  ...
  it("retries register until DS becomes available", async () => {
    ...
    const runtime = new AdapterRuntime(
      makeEnv(),
      makeDsClient(register) as unknown as DeviceServiceClient,
      makeStoreStub() as unknown as SqliteStore,
      {} as EventNormalizationPipeline,
      logger
    );
  ...
  it("re-registers when heartbeat returns adapter_inactive", async () => {
    ...
    const heartbeat = vi
      ...
      .mockRejectedValueOnce(
        new PermanentDeviceServiceError("inactive", {
          statusCode: 409,
    ...
    const runtime = new AdapterRuntime(
      makeEnv({ HEARTBEAT_INTERVAL_MS: 10 }),
      {
        register,
        heartbeat,
        ingestEvents: vi.fn().mockResolvedValue({ results: [] })
      } as unknown as DeviceServiceClient,
      makeStoreStub() as unknown as SqliteStore,
      {} as EventNormalizationPipeline,
      makeLoggerStub()
    );
...
function makeDsClient(register: ReturnType<typeof vi.fn>): {
...
function makeStoreStub(): {
  setLastAckedEventId: ReturnType<typeof vi.fn>;
  getLastAckedEventId: ReturnType<typeof vi.fn>;
  listPending: ReturnType<typeof vi.fn>;
  markAcked: ReturnType<typeof vi.fn>;
...
function makeLease(adapterId = "adapter-1"): AdapterLease {
  return {
    adapterId,
